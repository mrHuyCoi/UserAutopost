...
import {
  ...
  deleteIgnoredZalo,
  IgnoredConversation,
} from "../../services/ignoredZaloService";
import {
  getMyBotConfig,
  upsertMyBotConfig,
  BotConfig,
...
const ZaloTab: React.FC<ZaloTabProps> = ({ initialActiveTab }) => {
  ...
  const [selectedConversation, setSelectedConversation] =
    useState<ZaloConversation | null>(null);
  ...
  const [subTab, setSubTab] = useState<"messages" | "staff" | "ignored">(
    "messages"
  );
  ...
  const [deletingIgnoredId, setDeletingIgnoredId] = useState<string | null>(
    null
  );
  ...
  const recentlySentRef = useRef<
    Record<string, { text?: string; kind?: "text" | "image"; time: number }[]>
  >({});
  ...
  const addStaffFromConversation = async (conv: ZaloConversation) => {
    try {
      ...
      await createStaffZalo({
        zalo_uid,
        name,
        role: "staff",
        permissions: { can_control_bot: true },
        owner_account_id: selectedAccountId || undefined,
      });
  ...
  const loadSessions = async () => {
    try {
      ...
      if (
        !selectedAccountId ||
        !items.find(
          (s) => String(s.account_id || "") === String(selectedAccountId)
        )
      ) {
  ...
  const handleSendImageFile = async (fileOverride?: File) => {
    ...
    if (!threadId) {
      alert(
        "Không xác định được thread_id cho cuộc trò chuyện này, không thể gửi ảnh."
      );
      return;
    }
    ...
    try {
      ...
      // Record recently sent image for dedupe window BEFORE awaiting send
      {
        ...
        const now = Date.now();
        const windowMs = 4000;
        const arr = (recentlySentRef.current[tid] || []).filter(
          (it) => now - it.time <= windowMs
        );
        arr.push({ kind: "image", time: now });
        recentlySentRef.current[tid] = arr;
      }
      await sendZaloImageFile(
        String(threadId),
        fileToSend,
        undefined,
  ...
  const loadIgnored = async () => {
    ...
    try {
      const resp = await listIgnoredZalo({
        limit: 200,
        offset: 0,
  ...
  const saveBotConfig = async () => {
    ...
    try {
      const resp = await upsertMyBotConfig(
  ...
  const togglePermission = async (
    ...
    // Optimistic update
    const next = staffItems.map((s) =>
      s.id === id ? { ...s, [field]: checked } : s
    );
    setStaffItems(next);
    setSavingStaffId(id);
    try {
  ...
  const loadStaff = async () => {
    ...
    try {
      const resp = await listStaffZalo({
      ...
      setStaffItems(
        items.map((it) => ({
          ...
          can_receive_notifications:
            it.can_receive_notifications ??
            it.permissions?.can_receive_notifications,
        }))
      );
    } catch (e) {
  ...
  const normalizeQuote = (
    q: any
  ): { text?: string; author?: string; ts?: number | string } => {
    if (!q) return {};
    let obj: any = q;
    if (typeof q === "string") {
      try {
          ...
          if (parsed && parsed.type === "photo") {
            return {
              ...
              width:
                parsed.width ??
                (parsed.params
                  ? JSON.parse(parsed.params || "{}").width
                  : undefined),
              height:
                parsed.height ??
                (parsed.params
                  ? JSON.parse(parsed.params || "{}").height
                  : undefined),
              title: parsed.title,
  ...
  const handleSSEMessage = (data: QRResponse) => {
    ...
    switch (data.type) {
      ...
      case "SessionSaveError": {
        ...
        const benign =
          errMsg.includes("checkupdate") || errMsg.includes("logincookie");
        if (benign) {
  ...
  const loadConversations = async () => {
    ...
    try {
      ...
      if (!selectedAccountId) {
        const firstOwner =
          (data.items || [])
            .map((it: any) => it?.owner_account_id)
  ...
  const handleSendMessage = async (e?: React.FormEvent) => {
    ...
    if (!threadId) {
      alert(
        "Không xác định được thread_id cho cuộc trò chuyện này, không thể gửi tin."
      );
      return;
    }
    if (!text) return;
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (
          status?.ok &&
          (status.exists === true || status.exists === "true")
        ) {
          setStatus("SessionSaved");
    ...
    ws.onmessage = (event) => {
      ...
      if (payload.type === "zalo_message" && payload.data) {
        ...
        if (
          ...
          current.thread_id &&
          d.thread_id &&
          String(current.thread_id) === String(d.thread_id)
        ) {
          ...
          recentlySentRef.current[tid] = arr;
          const isSelfOut =
            !!d.is_self || String(d.direction || "").toLowerCase() === "out";
          if (isSelfOut) {
            ...
            const idx = arr.findIndex(
              (it) =>
                String(it.text || "").trim() === String(d.content || "").trim()
            );
            if (idx >= 0) {
  ...
  const addIgnoredFromConversation = async (conv: ZaloConversation) => {
    try {
      ...
      setIsIgnoring(true);
      if (!selectedAccountId) {
        alert("Vui lòng chọn tài khoản để chặn hội thoại.");
        return;
      }
      await upsertIgnoredZalo({
        thread_id,
        name: (conv as any).d_name || undefined,
                  ...
                  <div className="flex flex-col md:flex-row md:items-center gap-3 p-3 bg-white border rounded">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        onClick={async () => {
                          if (!selectedAccountId) {
                            alert("Chọn tài khoản để đăng xuất.");
                            return;
                          }
                          try {
                        ...
                        conversations.map((conv) => (
                          ...
                          >
                            <div className="flex items-center gap-2 mb-1 pr-8">
                              ...
                              {ignoredItems.some(
                                (it) => it.thread_id === (conv as any).thread_id
                              ) && (
                                <span title="Đã chặn">
                                  ...
                                  {(() => {
                                    ...
                                    return (
                                      <button
                                        className="w-full text-left px-3 py-2 text-sm hover:bg-gray-50 flex items-center gap-2 disabled:opacity-60"
                                        onClick={() =>
                                          addIgnoredFromConversation(conv)
                                        }
                                        disabled={isIgnoring}
                                      >
                                        <XCircle size={14} />{" "}
                                        {conv.type === 1 || conv.group_name
                                          ? "Không trả lời nhóm này"
                                          : "Không trả lời người này"}
                                      </button>
                                    );
                                  })()}
                                </div>
                            ...
                            {(conv.last_created_at || conv.last_ts) && (
                              <p className="text-xs text-gray-400 mt-1">
                                {conv.last_created_at
                                  ? formatDateTime(conv.last_created_at)
                                  : formatDateTime(conv.last_ts)}
                              </p>
                            )}
                          </div>
                        ))
                            ...
                            >
                              ...
                              >
                                {!msg.is_self &&
                                  ...
                                  msg.d_name && (
                                    <div
                                      className={`text-xs font-semibold mb-1 ${getColorClassForName(
                                        msg.d_name
                                      )}`}
                                    >
                                      {msg.d_name}
                                ...
                                {(() => {
                                  ...
                                  return (
                                    <div className="text-sm">{nc.text}</div>
                                  );
                                })()}
                                <div
                    ...
                    {selectedConversation && (
                      <>
                        <form
                          ...
                          className="p-3 border-t border-gray-200 flex gap-2 items-center"
                        >
                          <input
                            ref={fileInputRef}
                            type="file"
                          ...
                          <button
                            ...
                            disabled={
                              isSendingMessage || !newMessageText.trim()
                            }
                ...
                <div className="flex items-center justify-between mb-3">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    >
                      <RefreshCw
                        size={16}
                        className={isLoadingBotConfig ? "animate-spin" : ""}
                      />
                      Tải cấu hình
                    </button>
                    <button
                    ...
                    >
                      <RefreshCw
                        size={16}
                        className={isLoadingIgnored ? "animate-spin" : ""}
                      />
                      Làm mới danh sách
                    </button>
                  </div>
              ...
              <div className="bg-white rounded-lg shadow-sm p-4">
                <div className="flex items-center justify-between mb-3">
                  ...
                  >
                    <RefreshCw
                      size={16}
                      className={isLoadingStaff ? "animate-spin" : ""}
                    />
                    Làm mới
                  </button>
                </div>
                        ...
                        {staffItems.map((s) => (
                          <tr key={s.id} className="border-b hover:bg-gray-50">
                            ...
                            <td className="py-2 pr-4">
                              {s.is_active ? "Đang hoạt động" : "Đã vô hiệu"}
                            </td>
                  ...
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                      <div className="font-medium text-gray-800">
                        Cấu hình tạm dừng phản hồi
                      </div>
                      <div className="text-xs text-gray-500 mt-0.5">
                        Đặt số phút bot sẽ không trả lời tin nhắn mới (áp dụng
                    ...
                    {botConfig && (
                      <div className="text-xs text-gray-500 mt-2 sm:mt-0">
                        ...
                        {botConfig.updated_at
                          ? new Date(botConfig.updated_at).toLocaleString(
                              "vi-VN"
                            )
                          : botConfig.created_at
                          ? new Date(botConfig.created_at).toLocaleString(
                              "vi-VN"
                            )
                          : "Chưa có"}
                      </div>
